...
double voxel_leaf_size = 0.5;  // Leaf size of VoxelGrid filter.
...
void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  icp.setInputSource(filtered_points_ptr);
  ...
  //double transformation_probability = icp.getTransformationProbability();
  Eigen::Matrix4f t_localizer = icp.getFinalTransformation();
  ...
  std::cout << "processed time(s): " << (callback_end-callback_start).toSec() << std::endl;
